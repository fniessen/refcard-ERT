...
#+KEYWORDS:  emacs, ert, emacs lisp, regression, testing, test, tests
#+LANGUAGE:  en
...
To run the tests in batch mode, type (for example, for YASnippets):
...
# - ~M-x org-test-load~ ::
#      Loads up the Org mode test suite (all ~.el~ files in ~testing/lisp~).
...
support future development by making a [[https://www.paypal.com/cgi-bin/webscr?cmd=_donations&business=VCVAS6KPDQ4JC&lc=BE&item_number=refcard%2dERT&currency_code=EUR&bn=PP%2dDonationsBF%3abtn_donate_LG%2egif%3aNonHosted][donation]] through PayPal.
